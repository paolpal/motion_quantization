...
try:
    ...
    print(f"Errore: Moduli non trovati. {e}")
    exit(1)
...
STUDY_NAME = "vqvae_skeleton_tuning"
...
def objective(trial):
    config = {
        "init_lr": trial.suggest_float("init_lr", 5e-7, 1e-4, log=True),
        "dropout": trial.suggest_float("dropout", 0.05, 0.2),
        "num_codes": trial.suggest_categorical("num_codes", [32, 48, 64, 80, 96]),
        ...
        "threshold_ema": trial.suggest_int("threshold_ema_dead_code", 50, 200, step=50),
        "noise_factor": trial.suggest_float("noise_factor", 0.0, 0.01),
    }
    ...
    train_loader = DataLoader(GLOBAL_TRAIN, batch_size=config["batch_size"], shuffle=True, num_workers=4, pin_memory=True, drop_last=True)
    val_loader = DataLoader(GLOBAL_VAL, batch_size=config["batch_size"], num_workers=4, pin_memory=True)
    ...
    try:
        for epoch in range(1, 301):
            ...
            for batch in train_loader:
                ...
                optimizer.zero_grad()
                noise = torch.randn_like(batch) * config["noise_factor"]
                batch_noisy = batch + noise
                recon, vq_l, idxs = model(batch_noisy)
            ...
            avg_train_recon = train_recon / len(GLOBAL_TRAIN)
            avg_train_vq = train_vq / len(GLOBAL_TRAIN)
            avg_train = avg_train_recon + avg_train_vq
            
            avg_val_recon = val_recon / len(GLOBAL_VAL)
            avg_val_vq = val_vq / len(GLOBAL_VAL)
        ...
        # Optuna Metrics 
        val_clip_ids = np.unique(GLOBAL_VAL.clip_indices)
        stability_accum = 0.0
        recon_accum = 0.0
        ...
        with torch.no_grad():
            for c_id in val_clip_ids:
                clip = GLOBAL_VAL.clips[c_id].to(device)
                recon_clip, _, idxs = model(clip)
                
                stability_accum += calculate_stability_metric(clip, recon_clip).item()
                
                recon_accum += recon_criterion(recon_clip, clip).item()
                
                counts = torch.histc(idxs.float(), bins=config["num_codes"], min=0, max=config["num_codes"]-1)
                probs = counts / (counts.sum() + 1e-10)
                perp = torch.exp(-torch.sum(probs * torch.log(probs + 1e-10)))
                perplexity_accum += perp.item()
        
        avg_stability = stability_accum / len(val_clip_ids)
        avg_recon = recon_accum / len(val_clip_ids)
        perplexity = perplexity_accum / len(val_clip_ids)

        run.log({
            'val_stability_metric': avg_stability,
            'val_recon_clip_loss': avg_recon,
            'val_perplexity': perplexity,
        })

        model.save(Path(f"vqvae_trial_{trial.number}.pt"))
    finally:
        run.finish()

    return avg_stability, avg_recon, -perplexity/config["num_codes"]
...
if __name__ == "__main__":
    ...
    GLOBAL_TRAIN = MultiSpeakerDataset(speakers=speakers, split="train")
    GLOBAL_VAL = MultiSpeakerDataset(speakers=speakers, split="dev")

    study = optuna.create_study(study_name=STUDY_NAME, storage=DB_NAME, direction="minimize", 
                                load_if_exists=True, pruner=optuna.pruners.MedianPruner(n_warmup_steps=10))
